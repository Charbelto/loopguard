...
def run_simulation(use_guard):
    ...
    guard = LoopGuard(similarity_threshold=0.8, min_repeats=3, max_cycle_length=2) if use_guard else None
...
def main():
    ...
    for line in result_standard["trace"]:
        print(line)
    print(f"Outcome: {'SUCCESS' if result_standard['success'] else 'FAILURE (Max steps reached)'}")
    ...
    print(f"{'Total Execution Steps':<25} | {result_standard['steps']:<20} | {result_protected['steps']:<20}")
    print(f"{'Input Tokens Used':<25} | {result_standard['input_tokens']:<20} | {result_protected['input_tokens']:<20}")
    print(f"{'Output Tokens Used':<25} | {result_standard['output_tokens']:<20} | {result_protected['output_tokens']:<20}")
    print(f"{'Total API Cost ($)':<25} | ${result_standard['total_cost']:<19.4f} | ${result_protected['total_cost']:<19.4f}")
    
    cost_saved = result_standard["total_cost"] - result_protected["total_cost"]
    percent_saved = (cost_saved / result_standard["total_cost"]) * 100.0 if result_standard["total_cost"] else 0
    print("-" * 75)
    print(f"Efficiency Improvement: {percent_saved:.1f}% Cost Reduction (Saved ${cost_saved:.4f})")
    print("==================================================")
